...
export default {
  setup() {
    ...
    const { permissions } = storeToRefs(usersStore);
    const { sendAddUsers } = usersStore;
  ...
  methods: {
    async onSave() {
      ...
      try {
        ...
        await this.sendAddUsers(payload);

        this.showSnackbar(false, this.t("messages.createSuccess"));
